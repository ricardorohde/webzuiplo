...
(function($) {
  ...
  function Plot(placeholder, data_, options_, plugins) {
    ...
    function parseOptions(opts) {
      ...
      if (options.xaxis.color == null) options.xaxis.color = $.color.parse(options.grid.color)
        .scale("a", .22)
        .toString();
      if (options.yaxis.color == null) options.yaxis.color = $.color.parse(options.grid.color)
        .scale("a", .22)
        .toString();
      ...
      if (options.grid.tickColor == null) options.grid.tickColor = $.color.parse(options.grid.color)
        .scale("a", .22)
        .toString();
      ...
      for (i = 0; i < options.xaxes.length; ++i) getOrCreateAxis(xaxes, i + 1)
        .options = options.xaxes[i];
      for (i = 0; i < options.yaxes.length; ++i) getOrCreateAxis(yaxes, i + 1)
        .options = options.yaxes[i];
      for (var n in hooks)
        ...
        axis.tickFormatter = function(value, axis) {
          ...
          if (axis.tickDecimals != null) {
            ...
            if (precision < axis.tickDecimals) {
              return (precision ? formatted : formatted + ".") + ("" + factor)
                .substr(1, axis.tickDecimals - precision)
            }
          }
          return formatted
        }
        ...
        if (otherAxis && otherAxis.used && otherAxis != axis) {
          ...
          if (!axis.mode && opts.tickDecimals == null) {
            ...
            if (!(ts.length > 1 && /\..*0$/.test((ts[1] - ts[0])
                .toFixed(extraDec)))) axis.tickDecimals = extraDec
          }
        }
      }
    }
    ...
    function insertLegend() {
      ...
      if (options.legend.container != null) $(options.legend.container)
        .html(table);
      else {
        var pos = "",
